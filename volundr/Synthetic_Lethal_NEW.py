...
class SyntheticLethal:
    ...
    def tc_norm(self):
            ...
            for index_key in library_index_list:
                ...
                outdatafile = \
                    open("{}{}_{}_{}_Filtered_Targets.txt"
                         .format(self.args.Working_Folder, self.args.Job_Name, sample_name, index_key), 'w')
        ...
        '''
        # bad_correlation_outfile = open("{}{}_Bad_Correlation.txt".format(self.args.Working_Folder, self.args.Job_Name), 'w')
    ...
    def dictionary_builds(self):
        ...
        sample_data_dict = \
            {"Unknown": [[0] * (sample_index_mismatch + 1), 0, 0, 0, "Unknown", "Unknown", "Unknown"]}
        ...
        for sample in self.SampleManifest:
            ...
            sample_data_dict[index_name] = \
                [[0]*(sample_index_mismatch+1), 0, 0, 0, sample[0], sample[1], sample[2]]
        ...
        for target_name in self.target_dict:
            if self.args.Verbose == "INFO":
                break
            Tool_Box.debug_messenger([target_name, self.target_dict[target_name]])
    ...
    def fastq_processing(self):
        ...
        for i in range(self.args.Spawn):
            ...
            for sample_index in self.sample_data_dict:
                ...
                tmp_file_list.append([open("{}{}_{}.tmp".format(self.args.WorkingFolder, i, sample_index), "a")])
        ...
        while not eof:
            ...
            for i in range(self.args.Spawn):
                try:
                    fq1_group.append(next(fastq1.grouper()))
                except StopIteration:
                    eof = True

            count_increment += self.args.Spawn
            p.starmap(fastq_data.file_writer, zip(worker, fq1_group, tmp_file_list))

            if count_increment % 1000 == 0:
                gc.collect()
        ...
        for worker_id in worker:
            for sample_index in self.sample_data_dict:
                ...
                outstring = ""
                self.log.debug("Reading Temp FASTQ {}{}_{}.tmp".format(self.args.WorkingFolder, worker_id, sample_index))
                for line in r1_tmp_file:
                    outstring += line

                r1_out = gzip.open("{}{}_{}.fq.gz".format(self.args.WorkingFolder, self.args.Job_Name, sample_index), "a")
